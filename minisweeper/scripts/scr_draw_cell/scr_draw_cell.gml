...
if (_mask) {
	draw_sprite_ext(sMask,mip,_xx,_yy,_scale,_scale,0,backColor,1);
}
...
if (clearedGrid[# xx,yy]) {
	if (clearedColor == -1) {
		...
		draw_sprite_ext(sSquare,mip,_xx,_yy,_scale,_scale,0,clearedColor,1);
	}
	...
	if (removeEaseGrid[# xx,yy] < 1) {
		...
		if (solidColor == -1) {
			...
			draw_sprite_ext(sSquare,mip,_xx,_yy + _sScale*_farIn*4,_scale*_size,_scale*_size,_rot,solidColor,_alpha);
		}
	}
	...
	if (mineGrid[# xx,yy]) {
		...
		if (mineColor == -1) {
			...
			draw_sprite_ext(sMineUncolored,mip,_xx,_yy,_scale*_size,_scale*_size,0,mineColor,1);
		}
	}
	...
	if (solidColor == -1) {
		...
		draw_sprite_ext(sSquare,mip,_xx,_yy,_scale,_scale,0,solidColor,1);
	}
	if (lost) {
		if (mineGrid[# xx,yy]) {
			var _size = ease_backOut(0,1,mineEaseGrid[# xx,yy],1, 3.5);
